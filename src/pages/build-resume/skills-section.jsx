  ...
  return (
    <div>
      ...
      <div className="space-y-4 mt-4">
        {skills.map((skill, index) => (
          <Card key={index}>
            <div className={`grid gap-4 ${showExperienceLevel ? 'sm:grid-cols-[1fr_1fr_auto]' : 'sm:grid-cols-[1fr_auto]'} sm:items-center`}>
              ...
              {showExperienceLevel ? <LevelSelector onLevelChange={(value) => dispatch(modifySkill({ index, key: 'level', value }))} /> : null}
              <button
                type="button"
                onClick={() => dispatch(removeSkill({ index }))}
                className="hover:text-red-400 transition-colors p-2 rounded-lg hover:bg-slate-50 self-end mb-1 cursor-pointer"
        ...
        <PrimaryButton
          className="glass flex w-full mt-12 items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-white/70 py-3 text-sm font-medium transition hover:bg-white/70"
